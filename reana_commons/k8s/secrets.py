...
class REANAUserSecretsStore(object):
    ...
    def _initialise_user_secrets_store(self):
        ...
        try:
            ...
            current_k8s_corev1_api_client.\
                create_namespaced_secret(K8S_DEFAULT_NAMESPACE,
                                         empty_k8s_secret)
    ...
    def _dump_json_annotation_to_k8s_object(self, k8s_object, annotation_key,
        ...
        try:
            k8s_object.metadata.annotations[annotation_key] = \
                json.dumps(annotation_value)
        except TypeError as e:
            log.error('Could not add annotations to user secrets:\n'
                      '{}'.format(str(e)), exc_info=True)

    def _load_json_annotation_from_k8s_object(self, k8s_object,
                                              annotation_key):
        """Load string annotations from Kubernetes object."""
        try:
            return json.loads(k8s_object.metadata.annotations[annotation_key])
        except ValueError as e:
            log.error('Annotations for user {} secret store could not be'
                      'loaded as json.'.format(annotation_key))
        except KeyError as e:
            log.error('Annotation key {annotation_key} does not exist for'
                      ' user {user} secret store, so it can not be loaded'
                      .format(annotation_key=annotation_key,
                              user=k8s_object.metadata.name))
                ...
                if k8s_user_secrets.data.get(secret_name) and not overwrite:
                    raise REANASecretAlreadyExists(
                        ...
                        'If you want change it use overwrite'.format(
                            secret_name))
                secrets_types = \
                    self._load_json_annotation_from_k8s_object(
                        k8s_user_secrets, 'secrets_types')
                secrets_types[secret_name] = secrets_dict[secret_name]['type']
                self._dump_json_annotation_to_k8s_object(k8s_user_secrets,
                                                         'secrets_types',
                                                         secrets_types)
                k8s_user_secrets.data[secret_name] = \
    ...
    def get_secrets(self):
        ...
        for secret_name in secrets_store.data:
            secrets_types = \
                self._load_json_annotation_from_k8s_object(secrets_store,
                                                           'secrets_types')
