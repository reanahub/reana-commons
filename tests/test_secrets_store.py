...
def test_create_secret(user_secrets, test_user):
    """Test creation of user secrets."""
    corev1_api_client = Mock()
    corev1_api_client.read_namespaced_secret = Mock(
        side_effect=ApiException(reason='Secret does not exist.',
                                 status=404))
...
def test_overwrite_secret(corev1_api_client_with_user_secrets,
                          user_secrets, test_user):
