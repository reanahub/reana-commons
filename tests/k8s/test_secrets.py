...
def test_create_secret():
    ...
    with patch(
        ...
        secrets_store = UserSecretsStore.fetch(uuid4())
        secrets_store.add_secrets(secrets)
...
def test_create_existing_secrets_fail(
    ...
    with patch(
        ...
        secrets_store = UserSecretsStore.fetch(no_db_user)
...
def test_overwrite_secret(
    ...
    with patch(
        ...
        secrets_store = UserSecretsStore.fetch(no_db_user.id_)
        secrets_store.add_secrets(secrets, overwrite=True)
        UserSecretsStore.update(secrets_store)
